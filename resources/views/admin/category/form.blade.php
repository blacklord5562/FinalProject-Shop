...
@section('title')
    ایجاد / ویرایش گروه کالا
@endsection
@section('content')
    <div class="row">
        <form action="@if(isset($editing)){{route('category.update',['id'=>$editing->id])}}@elseif(!isset($editing)){{route('category.store')}}@endif" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="m-1">
